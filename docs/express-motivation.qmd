...
That's because, on startup, Express code gets executed once before the actual session gets established, then once for each session.[^execute-twice]
As a result, you have to check if the session object exists before using it.

[^execute-twice]: Another potential reason to prefer Shiny Core is that you have to worry about expensive UI code having execute multiple times.
