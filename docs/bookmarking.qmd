...
When clicked, this button triggers the bookmarking process. The button works together with your `@session.bookmark.on_bookmarked` callback:
...
- If you haven't defined an `on_bookmarked` callback, clicking the button will display the bookmark URL in a modal dialog
...
# Note: No @reactive.effect to trigger bookmarking automatically
...
::: {.callout-tip}
Use URL bookmarking if your state can fit in ~65k characters. It's simpler to set up and more portable. This is great for smaller apps. However, generative-AI apps or apps with data uploads are better suited for and may even require server-side bookmarking.
:::
