...
The basic example above illustrates the idea using minimal code, but the app itself is not particularly useful.
For brevity sake, we won't fully explore compelling applications here, but in general, updating inputs can be useful for:

* Navigating complex UI (i.e., lots of tabs, controls, etc)
    * Allow users describe to what they want without finding it

* Combine with [input suggestions](genai-chatbots.qmd#suggest-input) to:
