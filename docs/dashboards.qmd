...
ui.page_opts(title="Penguins dashboard")
...
Create a sidebar layout by just wrapping sidebar content in `ui.sidebar()`.
Most often, it's best to place [inputs](/components/#inputs) in the sidebar and [outputs](/components/#outputs) in the main content area.
...
Cards are great for visually grouping together related content.
It's often best to wrap an output (or multiple related outputs) in a `ui.card()`.
...
Many of Shiny's layouts and components automatically fill the screen when this option is set, which is often desirable for dashboards, but may not be desirable when certain content wants a fixed size (e.g., a value box or card with a textual description). In this situation, you can override this behavior on a per-component basis by setting `fill=False` (and/or specifying a `height`):
...
ui.page_opts(title="Restaurant tipping")
