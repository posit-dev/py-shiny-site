...
In this article, we'll look at how to make your Shiny apps more responsive by using non-blocking operations.
We'll also go out of our way to explain why the usual Python async techniques don't work the way you expect in Shiny.

## Using async/await in Shiny

Asynchronous programming is a technique used in Python to increase scalability and responsiveness, usually in networked apps.
...
With a small tweak, we can run the task on a different process instead of a different thread.
