...
::: {.callout-tip}
## Do you need a browser for this test?

End-to-end tests are the only way to check what the user actually sees, but they're also the slowest to run and the most fiddly to keep passing.
If a test only cares about what your *server* computes — that an output has the right value, that a bad input raises — the [`local_server` fixture](unit-testing.qmd#testing-the-server-function) can check it in memory, with no browser at all.
:::
...
```bash
# Anthropic (default)
uv run shiny add test --app app.py

# OpenAI (will use gpt-5)
uv run shiny add test --app app.py --provider openai

# explicitly use gpt-5-mini model
uv run shiny add test --app app.py --provider openai --model gpt-5-mini
```
...
- [Test mode](test-mode.qmd) - Assert on server-side state (inputs, outputs, and internal reactive values) in your end-to-end tests.
- [Unit testing](unit-testing.qmd#testing-the-server-function) - Test the same server-side state without a browser, with the `local_server` fixture.
