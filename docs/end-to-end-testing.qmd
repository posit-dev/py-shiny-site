...
If a test only cares about what your *server* computes — that an output has the right value, that a bad input raises — [`test_server()`](test-server.qmd) can check it in memory, with no browser at all.
:::
...
- [Server testing](test-server.qmd) - Test the same server-side state without a browser, using `test_server()`.
