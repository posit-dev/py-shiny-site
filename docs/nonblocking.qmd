...
See the [Task Button](/components/inputs/task-button/index.qmd) component page for the button's own options, including `label_busy` and resetting it manually with `ui.update_task_button()`.
:::
...
`ExtendedTask` operations are automatically traced when using [OpenTelemetry](opentelemetry.qmd). This lets you monitor background task execution, identify slow operations, and track performance in production without adding debug code.
:::
