...
You'll also commonly see Core imports like `from shiny import reactive, render` in Express apps, highlighting the fact that things like reactivity and rendering work the same way in both modes.
...
As your app starts to get more complex, you may find yourself wanting to switch from Express to Core.
Now that we've seen the [differences](#whats-the-difference) between the two modes, here's a summary of actionable tips to help translate from Express to Core:

1. Remove all `shiny.express` imports.
2. Add `from shiny import App, ui` to your imports.
