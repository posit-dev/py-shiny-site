...
*   **Every test gets a fresh app.** `local_server` is function-scoped, because a session remembers every input set so far — sharing one across tests would let them leak into each other.
*   **The app runs unchanged.** No `test_mode=True`, no `conftest.py`, no fixture of your own.
...
*   **`timeout_secs=`** (default `5.0`), the cap on how long any single reactive flush may take before raising `TimeoutError`. Raise it if the app is simply slow to start — a cold matplotlib font cache, say.
