...
Let's modify our work above to use the Lazy API.
```{.python filename="load_data.py"}
import polars as pl
from pathlib import Path
...
You might notice that the syntax has changed a little bit. We've used   been using (ex. `.collect()`) looks different than what you are familiar with. That's because we're using a special feature of Polars called the *lazy API*. We'll talk more about that in the next section.




Let's look at an example with the [The Weather Dataset](https://www.kaggle.com/datasets/guillemservera/global-daily-climate-data). This dataset has


# TODO: Test this with Weather and update it
```python
