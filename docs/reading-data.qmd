...
From local files, databases, cloud-hosted data stores, and beyond, if you can read your data into Python, you can also read it into Shiny. Here we'll highlight some of our recommended ways to read data, which can be grouped into two main categories: [eager](#eager) and [lazy](#lazy).
...
Depending on the provider, you will have to set up the appropriate credentialing (more info [here](https://docs.pola.rs/user-guide/io/cloud-storage/#cloud-authentication)). Once you've done so, reading from the cloud is just like reading from a file on disk.

```python
import polars as pl

source = "s3://bucket/yourfilehere.parquet"
...
::: callout-tip
### Reading from a file stored in the cloud

Polars also supports lazy reading from cloud storage using its query optimization tooling. This is very helpful for limiting how much information is downloaded to memory, especially if you only need a small portion of a much larger dataset for your application (ex. You have data from 1900-present but you only need 2000-present.)

```python
import polars as pl

source = "s3://bucket/yourfilename.parquet"

# Polars will perform filtering, selection, and other manipulations here and only pull down
# the portions of the dataset you actually need from the remote store
df = pl.scan_parquet(source).filter("your filter here").collect()
```
:::
