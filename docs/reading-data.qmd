...
These strategies will serve us well with small to medium datasets. However, as our data gets larger, it becomes more critical to optimize what we are keeping in memory at any given time and when and where our computations are performed.

In the next section, we'll talk through strategies for working with larger data and remote datastores.

## Databases and larger-than-memory data {#read-db}
Some datasets are too large to fit cleanly in memory or don't live on our computer. Perhaps you want to connect to a remote database and/or perform complicated filtering operations on a large dataset. In these cases, knowing when you are loading data into memory and where and when you are performing calculations becomes more critial.

You'll notice a common pattern among toolsets for working with large and/or remote data. First, you make a connection to the data store. Then, you write some kind of operation you wish to perform on that data (ex. fetch 100 rows, fetch the top 3 records across each group, etc.). Finally, you execute that operation and read the result into memory.

There are two general types of tools you can use to make working with large data fast and efficient:

1. Tools like Polars' Lazy API, which perform calculations only when needed and with internal optimization tooling. You can load massive datasets from storage on-disk using formats optimized for large data, like Parquet.

2. Tools for interacting with remote databases. These tools let you query a database and and bring only what you need into memory. These tools include Ibis, SQLAlchemy, and other database connection libraries.
...
another_result = df.filter(pl.col("colname") == "value").collect()
```
...
# Ibis combines the query and execute stages we described above in one step
query_result = my_table.head(100)

# In other cases, we may want to separate these steps and call `execute()` explicitly
another_result = df.filter(df.colname == "value").execute()
...
with engine.connect() as conn:
    query_result = conn.execute(text("SELECT * FROM tablename LIMIT 100"))

    another_result = conn.execute(text("SELECT * from table where colname='value'"))
```
:::

In all cases, we define our query (our data manipulations) and then rely on an external 'engine' to optimize it, run it, and provide us with the results, at which point those results are loaded into memory.

We'll go over two more complicated examples below.
...
def results_df():
    ...
    return render.DataGrid(df.head(100).collect())
```
