...
There's also the `app.enter_context(pool)` line at the end of each example.
This is necessary to ensure that the pool is shut down when the app stops running.
In particular, if you're using `ProcessPoolExecutor` and neglect to shut down the pool, you may end up with orphaned Python processes hanging around.
