...
The `state.values` dictionary works with both URL and server-side bookmarking (configured via `bookmark_store`). It's automatically serialized when a bookmark is created and made available to your app during restoration through the `on_restore` and `on_restored` callbacks.
...
async def _(state):
    # Use reactive.isolate() if you need to avoid creating dependencies
...
- `@session.bookmark.on_bookmark` - Called **before** saving state. Use this to add custom values to `state.values`. For URL bookmarking, values are serialized into the URL query string. For server-side bookmarking, values are written to a file on the server.
...
- `@session.bookmark.on_restored` - Called **after** the session is fully initialized. Use this for operations that require a fully initialized application.
...
def bookmark_url():
    url = current_url()
    if url:
        return f"Current bookmark URL:\n{url}"
    return "No bookmark created yet"
...
@session.bookmark.on_bookmarked
async def _(url: str):
    current_url.set(url)  # Store for display
    await session.bookmark.update_query_string(url)
```
:::
