...
- `@session.bookmark.on_bookmark` - Called **before** saving state. Use this to add custom values to `state.values`. The final `state.values` is used for the bookmarked state. For URL bookmarking, values are serialized into the URL query string. For server-side bookmarking, values are written to a JSON file on the server.
