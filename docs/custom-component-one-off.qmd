...
It is required by Shiny `Renderer` that the `transform` be `async`. This allows for asynchronous transformations to occur even if the output value function is synchronous. By starting as an asynchronous function, we can be sure that we can handle any type of output value function.
:::
