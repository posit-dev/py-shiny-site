...
#### Shiny Core separates UI and server code
...
#### Shiny Core passes children as args
...
#### Shiny Express wraps children in a `with` block
...
- The `with` block syntax is slightly more readable, as it depends only on indentation and not on parentheses and commas.
