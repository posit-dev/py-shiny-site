...
Before jumping into the tools and techniques for building Generative AI Shiny apps, it's helpful to see some inspiring examples of what's possible.
Below are a few just examples of Shiny apps built with [components](../components/) like `Chat()` and `MarkdownStream()`.
Make sure to also check out the [templates](../templates/) for more examples of leveraging Generative AI in Shiny apps.

## First, some realism

TODO: Joe to impart his wisdom here?
...
LLMs are also very good at extracting [structured data](genai-structured-data.qmd) from unstructured text, which can be useful for a wide variety of tasks.
...
::: callout-note
This particular example is available as a [template](../templates/):
...
A more advanced use of this concept is to drive multiple views of the data with a single natural language query.
This [hosted app](https://jcheng.shinyapps.io/sidebot/) ([source](https://github.com/jcheng5/py-sidebot)) applies that idea to the restaurant `tips` dataset:

![Screenshot of the "sidebot" app, which leverages LLMs to generate SQL queries that match a user's natural language query.](/images/genai-sidebot.png){class="rounded shadow lightbox mt-3"}

This app also leverages the LLMs ability to "see" the visuals.
By clicking on the ✨ icon, the user is provided with a natural language description of the visualization, which can be useful for accessibility or for users who are not as familiar with the data.
