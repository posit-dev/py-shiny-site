...
**Maturity is no longer a strong reason to prefer one syntax over the other.**

Shiny Core has the longer history: it has been Shiny for Python's syntax since the first release, while Shiny Express arrived in version 0.7.0 (January 2024).
Since then, the Express syntax has stayed stable.
Its only Express-specific breaking change was a more ergonomic signature for `express.ui.insert_accordion_panel()` in version 1.5.0.

Both syntaxes are built on the same reactive engine, UI components, and render functions, and they ship and are tested together in every release.
