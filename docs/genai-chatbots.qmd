...
```bash
uvx shiny create --template chat-ai-ollama
```

### Anthropic

```bash
uvx shiny create --template chat-ai-anthropic
```

### OpenAI

```bash
uvx shiny create --template chat-ai-openai
```

### Google

```bash
uvx shiny create --template chat-ai-gemini
```

### Bedrock Anthropic

```bash
uvx shiny create --template chat-ai-anthropic-aws
```

### Azure OpenAI

```bash
uvx shiny create --template chat-ai-azure-openai
```

### LangChain

```bash
uvx shiny create --template chat-ai-langchain
```
...
```bash
uvx shiny create --template chat-ai-playground
```
:::
...
When a Shiny app reloads, the app returns to it's original state, unless the URL includes [bookmarked](../api/core/Session.qmd) state.[^1]
...
```bash
uvx shiny create --template data-sci-adventure \
...
For example, in the "What's for Dinner?" app, when the user requests to extract recipe, a message is shown with a human readable version of recipe, as well as a [download button](../api/express/express.render.download_button.qmd) to get the recipe in a structured JSON format:
...
```bash
uvx shiny create --template dinner-recipe \
    --github posit-dev/py-shiny-templates/gen-ai
```
:::
...
@render.download_button(filename="messages.json", label="Download messages")
def download():
...
When deploying AI chatbots to production, [OpenTelemetry tracing](opentelemetry.qmd) provides valuable insights:
