...
1. `chat` represents the chatbot UI.
    - Here you'll find methods for working with the chat's state (e.g., `.append_message()`)
    - `chat.ui()` puts a UI element on the page. Here you can specify [startup messages](#startup-messages), [customize icons](#custom-icons), and more.
...
3. `@chat.on_user_submit` accepts a callback to fire when the user submits input.
    - Here, `user_input` is passed to `chat_client.stream_async()` for response generation. The async stream helps to keep the chat app responsive and scalable.
...
[The provided templates](#choose-a-template) mostly use `chat_client.stream_async()` to generate the response and passes that result directly to `chat.append_message_stream()`.

As it turns out, `chat.append_message_stream()` works with any generator (or iterable) of strings.
That's useful to know if you want to use a different `chat_client` and/or you want to transform the response before it's sent to the chat.
...
As we'll see later, there a couple other ways to append messages to the chat, like `chat.append_message()` and `chat.append_message_stream()`.
--->
...
This particular example of updating user input uses a feature (that is, including Shiny UI components like inputs inside of messages) that is currently in development and not yet available on PyPI.
Install the latest version from the `main` branch on GitHub to use it.

```bash
pip install git+https://github.com/posit-dev/py-shiny.git@main
```
:::
...
![Screenshot of a chatbot with a input components updating the user input.](/images/chat-input-updating.png){class="rounded shadow lightbox d-block m-auto" width="67%"}
...
The previous section illustrated an interactive message that updates the user input based on the state of some input fields in a startup message.
Here's another example that displays a table of data and updates the user input based on the selected row:
...
with ui.hold() as data_grid_ui:

    @render.data_frame
    def data_grid():
        return render.DataGrid(dat, selection_mode="row")
...
@reactive.effect
def _():
    row = data_grid.cell_selection()["rows"]
    if not row:
        return
    name = dat.loc[row, "Name"]
    age = dat.loc[row, "Age"]
    chat.update_user_input(value=f"Name: {name} | Age: {age}")
```
:::
...
An alternative way to append a streaming messages is through the `.message_stream_context()` context manager, which is primarily useful for two things:

1. Nesting a stream within another.
2. Overwriting/replacing streaming content.
...
As you'll learn in [tool calling](genai-llms.qmd#tool-calling), a `.message_stream()` can also be nested inside an non-blocking `.append_message_stream()`, which is primarily useful for showing tool progress/results.
...
The next article covers a more generic way to stream generative AI: the `ui.MarkdownStream()` component.
This component is useful when you don't want a full chat interface, but still want to stream in some content from an LLM.

Skip to other articles in the series if you want to learn about other Generative AI techniques (that can power a `ui.Chat()` or a `ui.MarkdownStream()`) like [tool calls](genai-tools.qmd), [structured output](genai-output.qmd), and more.
