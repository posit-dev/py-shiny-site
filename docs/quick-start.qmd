...
* This example happens to use `shiny.express` which, [compared to core Shiny](express-motivation.qmd), reduces the amount of code required.
...
Shiny includes a wide variety of these decorators in its `render` module, but Shiny [extensions](custom-component-one-off.qmd) like [shinywidgets](jupyter-widgets.qmd) provide additional decorators for rendering other kinds of outputs, like [Jupyter Widgets](jupyter-widgets.qmd).
...
Shiny supports [Jupyter Widgets](https://ipywidgets.readthedocs.io/en/stable/) through the `shinywidgets` package.
...
However, if you're versed in front-end development, you can also use Shiny to create [custom components](custom-component-one-off.qmd) that leverage your favorite JavaScript framework.
