...
::: {.callout-tip}
## Reading exports without a browser

Exported values aren't only for end-to-end tests.
The [`local_server` fixture](unit-testing.qmd#testing-the-server-function) runs the server function in memory and reads the very same values with `get_export()`:
...
So a value you export for your Playwright tests is testable from a fast, browserless test too — no `test_mode=True` and no running app required.
:::
