...
In [Unit testing](unit-testing.qmd#testing-the-server-function), you read a session's inputs, outputs, and exported values in-process, through `local_server`.
Those readers work because the session keeps a *snapshot* of the server's state.
**Test mode** is what makes that same snapshot available from a *running* app: with it enabled, a Shiny session serves the snapshot as JSON over HTTP, so an [end-to-end test](end-to-end-testing.qmd) can check server-side values from a real browser session.

That matters because Playwright controllers only see what the user sees.
Sometimes you want to check a value your app doesn't render at all — the result of a `reactive.calc` — or read an output's value without parsing it out of the page's HTML.

The snapshot has the same three blocks you met in-process:

*   **`input`** — the current value of every input
*   **`output`** — the current value of every output
*   **`export`** — internal values you explicitly export from your server code with `export_test_values()`
...
(The in-memory `local_server` fixture and `test_server()` enable it for their own session the same way, which is why `get_export()` works there without any setup.)

### Exporting internal reactive values

By default, the snapshot only contains inputs and outputs. To surface an *internal* value — say, a `reactive.calc` that never gets rendered directly — call `export_test_values()` in your server code, exactly as in [Exported values](unit-testing.qmd#exported-values). Nothing changes on the app side between the two kinds of test; the same export serves both. The app below is the one the rest of this article tests:
...
Preprocessors are part of building the snapshot, not of serving it, so they apply equally to the values `local_server` reads in-process: a scrubbed output is scrubbed in `get_output()`, and a scrubbed input in `get_input()`.
