...
With `chatlas`, it's very easy to switch between the model and system prompt behind your `chat_model`.
...
We can still learn about the `chat` UI features without knowing much about LLMs.
...
This can be a helpful way to introduce the chatbot to the user and provide some guidance on how to get started.
...
Every `chat` instance should have an `@chat.on_user_submit` callback.
This is where you'll receive the user's input, generate a response, and append it to the chat.
Just like [the templates](#choose-a-template), most apps will use `chat_model.stream_async()` to generate the response and `chat.append_message_stream()` to append it to the chat.

As it turns out, `chat.append_message_stream()` works with any generator (or iterable) of strings.
For example, let's create a basic generator that just splits the user input into words and streams them back to the chat:
...
And since `.append_message_stream()` works with any generator, you can "wrap" a generator to transform the stream before it's sent to the chat.
...
Although input suggestions are a great starting point for recommending input to user, you want to design custom UI to update the user input.
...
This feature is currently in development and not yet available on PyPI.
...
Chat messages can contain an Shiny UI elements, like [inputs](../components/#inputs), [outputs](../components/#outputs), [display messages](../components/#display-messages), and [layouts](../layouts/).
This allows you to create interactive messages that can be used to collect user input, display rich interactive output, or provide additional context.
...
Here's another example that displays a table and uses the selected row to update the user input:
...
<!--- TODO: render a table with DataGrid() -->
