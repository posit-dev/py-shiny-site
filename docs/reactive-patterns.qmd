...
The `@reactive.event()` decorator restricts re-exectue to only changes in one (or more) reactive dependency. Any other reactive dependencies inside the function being decorated are ignored.
...
Sometimes you want to require an input before executing a reactive function and/or displaying output. The `req()` is designed for this purpose. It effectively stops execution until the statement provided is truthy.
...
from shiny.express import input, renderm ui
...
Sometimes its useful invalidate a reactive function on a schedule.
...
Sometimes its useful invalidate a reactive function when a file changes.
