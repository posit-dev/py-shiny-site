...
```bash
uvx shinylive export myapp1 site --subdir app1
uvx shinylive export myapp2 site --subdir app2
```
...
* Run `uvx shinylive export myapp site --full-shinylive`. This will cause it to include all of the Python packages from the Shinylive distribution.
...
```bash
uvx shinylive@latest export myapp site   # Export with the latest shinylive
uvx shinylive@latest assets cleanup     # Remove older cached shinylive files
```
`uvx` reuses the shinylive version it has cached, so `@latest` is what fetches a newer release.
:::
...
If you would like to deploy to a Posit Connect server, add your server with `rsconnect add` as described in the [Deploy](deploy-on-prem.qmd) page.
Then you can deploy the application as a static website:

```bash
uvx --from rsconnect-python rsconnect deploy html site
```
