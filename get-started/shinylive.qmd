...
```bash
uv run shinylive export myapp site
```
...
```bash
uv run shinylive export myapp1 site --subdir app1
uv run shinylive export myapp2 site --subdir app2
```
...
* Run `uv run shinylive export myapp site --full-shinylive`. This will cause it to include all of the Python packages from the Shinylive distribution.
...
```bash
uv pip install --upgrade shinylive
uv run shinylive assets remove   # Remove old cached shinylive files
```
