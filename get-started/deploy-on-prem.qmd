...
1.  Install the [rsconnect-python](https://github.com/posit-dev/rsconnect-python) package.

    ``` bash
    uv pip install rsconnect-python
    ```

    Deploying [Shiny Express](https://shiny.posit.co/blog/posts/shiny-express/) applications requires version 1.22.0 or later of the [rsconnect-python package](https://github.com/posit-dev/rsconnect-python).
...
3.  At the command line add a nickname for your connect server with `rsconnect add`.
    ...
    ``` bash
    uv run rsconnect add -n my-connect -s <server_url> --api-key <YOUR_KEY>
    ```

4.  Deploy your application with `rsconnect deploy shiny`

    ``` bash
    uv run rsconnect deploy shiny -n my-connect
    ```
...
(Don't forget that you need to [install shiny](install.html) and any other Python packages needed by your app(s), into whichever Python installation or virtualenv(s) you intend to use.)
