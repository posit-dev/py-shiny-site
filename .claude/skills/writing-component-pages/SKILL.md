...
|------|-------------|-------|
| `app-preview.py` | Gallery-list card (recorded to `thumbnail.png` + `preview.gif`) | Core, module-level `app_ui`, viewport-filling (`vh-100 d-flex justify-content-center align-items-center`) |
...
naming the tooling recognizes: the shinylive-link generator, the gallery recorder,
...
**`app-preview.py` and `app-detail-preview.py` are Core style** (module-level `app_ui`) by
convention, so every preview app reads the same way.
...
sidebar: components
appPreview:
  file: components/inputs/task-button/app-preview.py   # repo-root-relative path; puts the page in the gallery
listing:
- id: example
...
- `appPreview:` is what lists a page in the `/components/` gallery; the card itself shows
  the recorded `thumbnail.png`/`preview.gif`, so a new page also needs a `Spec` in
  `scripts/record-component-previews.py`. (A few older pages have an inline `preview:`
  HTML string instead; the gallery only checks that it exists, so don't add new ones.)
...
- `relevant-functions` `href` and `signature` are **generated, not hand-written**. Add the
  ...
  then run `make docs-update-relevant-functions` — it reads the quartodoc-generated `api/**`
...
- **List the component's mutator functions in `relevant-functions`, not just its
  ...
  `title:` for each and let `make docs-update-relevant-functions` fill in the rest.
...
```bash
make docs        # = docs-update-shinylive-links + docs-update-relevant-functions
make docs-gallery-previews NAMES="<name>"   # record thumbnail.png + preview.gif
...
- Core: `ui.insert_accordion_panel(id, panel, ...)` — pass a pre-built `ui.accordion_panel(...)`.
- Express: `ui.insert_accordion_panel(id, panel_title, *panel_contents, panel_value=..., ...)`
  — pass the title and body directly; there is no panel object, and in Express
  `ui.accordion_panel` is a context manager (one positional arg), so the Core form
  raises `TypeError` at click time.
...
1. **Verify first.** Wait for the deploy to finish, then load each new/changed component
   ...
   Actually **click every button / toggle every control** in each live Preview app and
   confirm the promised effect happens with no error (see "Exercise every interactive
   control" above). Report anything broken instead of handing over links to a broken page.
...
| `app-preview.py` written in Express style | Rewrite as Core with module-level `app_ui` |
| Hand-writing / pasting `shinylive:` values | Add the key with a placeholder, run `make docs-update-shinylive-links` |
| Non-Preview example app missing its `shinylive:` key | Add the key (script skips apps without it and only warns) |
| Inline `preview:` HTML on a new page | Use `appPreview: { file }` and record a gallery preview |
