  ...
  contents:
    - title: Tooltips
      ...
      details: |
        ...
        A tooltip is a box that appears next to an element when a user hovers over the element. To add a tooltip to a UI component, wrap the component in [`ui.tooltip()`](TODO ADD LINK TO UI TOOLTIP). Then pass `ui.tooltip()` one or more elements to display, such as a simple string that contains a message.
        ...
        Compare tooltips to [popovers](TODO ADD LINK TO UI.POPOVERS), which are a similar device for organizing the layout of a Shiny app.

        ## See Also

        [Modal messages](TO DO ADD LINK TO MODAL DETAIL PAGE) and [help text](TO DO ADD LINK TO HELP TEXT) provide a similar, but alternative way to display information to the user.
