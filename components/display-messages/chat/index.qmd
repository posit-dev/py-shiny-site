...
[Input suggestions](#input-suggestions) are a great tool for guiding user input, but sometimes you may want full programmatic control over it.
...
things = ui.input_checkbox_group(
    "things", None,
    choices=["Thing 1", "Thing 2", "Thing 3"],
)

value = ui.input_slider("value", None, 1, 10, 5)
...
@reactive.effect
def _():
    things = "None" if not input.things() else ", ".join(input.things())
...
2. Check the `.status()` of the stream.
3. `.cancel()` the stream.

Below is a basic example of reading the status and result of a streaming message:
...
And an example of canceling a stream:
...
The `chat` component stores messages and makes them available via the `chat.messages()` method.
However, it only stores the content shown in the UI, so if there is important background information (like tool calls, prompts, etc.) that you need to save/restore, you'll need to handle that separately.
With `chatlas`, you can easily save and restore chat history by using the `.get_turns()` and `.set_turns()` methods on the `chat_model`.

The example below demonstrates a basic approach to saving and returning to a previous chat session.
For a real chatbot that uses an LLM, you likely want to 
