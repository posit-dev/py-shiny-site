...
# NOTE: this workaround can be removed once this is merged
# https://github.com/posit-dev/py-shiny/pull/1868
@render.ui
def bind():
    return ui.tags.script(
        "await Shiny.initializeInputs(); await Shiny.bindAll(document.getElementById('chat'))",
        type="module",
    )
```
...
See the examples in the sections below to see them in action:
...
Beware that `chat.messages()` only returns the messages displayed in the UI, not the messages sent/returned by the LLM.
This means, if your chat history contains "background" context like tool calls, extra prompt templating, etc., you'll need to handle that separately.
Note that with `chatlas`, you can easily save and restore that additional context via the `.get_turns()` and `.set_turns()` methods on the `chat_model`.
:::
...
For a more involved example of how you can combine reactivity with `chat.messages()` to add a "New chat" button and a dropdown to select previous chats, see the example below:
