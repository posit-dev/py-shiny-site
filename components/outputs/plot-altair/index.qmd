...
- id: variations
  ...
  template-params:
    dir: components/outputs/plot-altair/
  contents:
  - title: Get hover over point information
    description: "Captures the hovered point in Altair and reactively displays the dataframe data."
    apps:
    - title: Preview
...
From there, the `.selections` attribute contains the selected points from the Altair figure as an [Altair `IndexSelection` object](https://altair-viz.github.io/user_guide/interactions/jupyter_chart.html#index-selections).
The selected points can be captured as reactive values with the `shiny.widgets_reactive_read()` function,
the `.value` attribute of the `IndexSelection` object contains the indices of the selected points.
