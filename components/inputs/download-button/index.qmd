...
- id: relevant-functions
  ...
  - title: ui.download_button
    ...
    signature: ui.download_button(id, label, icon=None, **kwargs)
  - title: ui.download_link
    href: https://shiny.posit.co/py/api/ui.download_link.html
    signature: ui.download_link(id, label, icon=None, **kwargs)
...
To add a download button to your app:
  ...
  1. Add `ui.download_button()` or `ui.download_link()` to the UI with a unique `id` and descriptive `label`.

  2. In the server function, create a function with the same name as the button's `id` and decorate it with `@render.download()`.
