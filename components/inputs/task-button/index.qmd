...
To add a task button to your app:
  ...
  2. In the server function, create an async function decorated with `@reactive.event(input.<button_id>)` (Core) or `@ui.event(input.<button_id>)` (Express) that performs the long-running operation.
