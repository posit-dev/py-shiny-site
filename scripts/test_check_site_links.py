"""Unit tests for scripts/test_site_links.py, the internal-link checker.

Not collected by the test-apps-* targets (pytest.ini scopes testpaths to
components/). Run explicitly:

    uv run pytest scripts/test_check_site_links.py
"""
...
# Loaded by path rather than imported: `make test-site-links` runs this file as a
# script, and pytest would otherwise import it a second time under its own name.
_SPEC = importlib.util.spec_from_file_location(
    "site_links", Path(__file__).with_name("test_site_links.py")
)
assert _SPEC and _SPEC.loader
site_links = importlib.util.module_from_spec(_SPEC)
sys.modules["site_links"] = site_links
_SPEC.loader.exec_module(site_links)
