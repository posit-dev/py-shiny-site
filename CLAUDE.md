...
- **Merge gating:** `done-site` is the only `site.yml` job in the required-checks list. It is an `if: always()` aggregator over `[build, merge-shards]` that fails on any `failure|cancelled|skipped` result, so the shard count and job layout can change freely without touching branch protection. The `always()` matters: a job with a plain `needs:` *skips* when an upstream job fails, and GitHub reports a skipped job as "Success" even when required — `done-site` turns that silent pass into a hard failure. `publish` is deliberately excluded from its `needs` and must **never** be required: it needs Netlify secrets and `deployments: write`, neither of which fork pull requests get, so gating on it would permanently block external contributors.
