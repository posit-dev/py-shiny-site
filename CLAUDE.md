...
- **CI caches:** `.quarto/shinylive-cache/` persists across runs via `actions/cache` (content-addressed keys; safe to restore stale). Two more caches exist to keep CI off github.com's release-asset throttle (see below): the Quarto install (`~/.local/share/quarto-<version>`, keyed on the Makefile's `QUARTO_VERSION`) and the shinylive web-assets bundle (`~/.cache/shinylive`, keyed on `requirements.txt`).
