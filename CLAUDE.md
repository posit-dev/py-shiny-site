...
Every `done-*` aggregator is `if: always()` plus one step: `./.github/internal/verify-job-results` with `results: ${{ toJSON(needs.*.result) }}`. The action fails on `failure|cancelled|skipped`, and its description records why `always()` and the `skipped` case are both load-bearing. `needs.*` expands to whatever is in the calling job's `needs:`, so adding a job there needs no other change. A new `done-*` job should reuse the action rather than re-inline the grep.
