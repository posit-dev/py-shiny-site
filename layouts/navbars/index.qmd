...
::: {.callout-tip}
A few related functions are useful when building navbars:

  - `ui.navbar_options()` bundles appearance options (e.g., `position`, `bg`, `underline`) to pass to the `navbar_options` argument of `ui.page_navbar()`.
  - `ui.nav_spacer()` adds a spacer between nav items, for example to push subsequent items to the right side of the navbar.
  - `ui.navset_bar()` creates a navbar *inside* a page (e.g., within `ui.page_fluid()`), rather than as the page layout itself, and accepts the same nav items as `ui.page_navbar()`.
:::
