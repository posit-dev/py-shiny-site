...
Sometimes you don't want Shiny to build the page at all — you already have a complete HTML document, such as the `index.html` your JavaScript bundler emits, or a design handed to you by someone else.
...
-   In Express mode, pass the document to `ui.page_opts(html=)`, which routes the whole app through `ui.page_html()`.
    Top-level UI markup you write in the app file is dropped — the document already *is* the page — but the HTML dependencies of those elements are kept.
