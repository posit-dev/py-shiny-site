...
- id: relevant-functions
  ...
  - title: ui.page_html
    href: https://shiny.posit.co/py/api/core/ui.page_html.html#shiny.ui.page_html
    signature: ui.page_html(html, *, extra_deps=None, deps_replace_pattern=DEPS_PLACEHOLDER)
---
...
from docs.helpers import app_preview_code, express_core_preview, shinylive_app_preview
```
...
## Bringing your own HTML page

Sometimes you don't want Shiny to build the page at all, because you already have a complete HTML document, such as the `index.html` your JavaScript bundler emits, or a design handed to you by someone else.
`ui.page_html()` serves that document as-is: you own the markup, and Shiny only fills in its own HTML dependencies and wires up any elements whose `id` matches an input or output.
...
shinylive_app_preview(
    ["app-page-html-core.py", "custom-page.html"], viewer_height=220, div_attrs=".p-0"
)
app_preview_code(
    {"Express": "app-page-html-express.py", "Core": "app-page-html-core.py"},
    files=["custom-page.html"],
    div_attrs='.shiny-mode-tabset group="shiny-app-mode"',
)
```
:::

The document itself is ordinary HTML:

```{python}
#| output: asis
#| echo: false

print('```{.html filename="custom-page.html"}')
print(open("custom-page.html").read().strip())
print("```")
```

A few things to know:

-   Pass the document as a string, or as a `pathlib.Path` to an HTML file (read fresh on every call).
    Use `App(ui=lambda request: ui.page_html(...))` rather than `App(ui=ui.page_html(...))` if your app supports [bookmarking](/docs/bookmarking.qmd), which requires a UI function.

-   Shiny's HTML dependencies, plus anything you pass to `extra_deps=`, are inserted where the document has `<meta name="shiny-dependency-placeholder" content="">`.
    If your document marks that spot some other way, name it with `deps_replace_pattern=`.
    Without a placeholder Shiny has nowhere to put its CSS and JavaScript, and the page won't be reactive.

-   In Express mode, pass the document to `ui.page_opts(html=)`, which routes the whole app through `ui.page_html()`.
    Top-level UI markup you write in the app file is dropped, since the document already *is* the page, but the HTML dependencies of those elements are kept.
