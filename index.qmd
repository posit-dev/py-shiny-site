...
<div class="gradient-text display-4 fw-bolder pb-4">Effortless Python web applications</div>


#### with the power reactive programming.
...
::::{.column-page .pt-5 .mt-5}
:::{.grid}
:::{.g-col-12 .g-col-md-4 .feature-section}

### Efficient

Shiny leverages a reactive execution engine to minimize rerendering of your application.
Outputs are rendered on-demand and only update when their upstream dependencies change.
With Shiny, you can create anything from a simple dashboard to a comprehensive web application within a single framework.
...
Rather than defining callback functions and state variables, Shiny automatically determines the best execution path for your app at runtime.
This allows you to focus more on delivering insights and less on troubleshooting execution issues.
...
Shiny is built on top of the modern Python web stack, using [Starlette](https://www.starlette.io/) and [asyncio](https://docs.python.org/3/library/asyncio.html).
The framework fully supports [CSS](docs/ui-customize.html) and [JavaScript](docs/custom-component-one-off.qmd) customization, as well as [asyncronous programming](docs/nonblocking.html).
You have lots of deployment options including our own [cloud hosting](docs/deploy-cloud.qmd), [self-hosted server](docs/deploy-on-prem.qmd), or [open source options](docs/deploy-on-prem.html#open-source-options).
...
[Documentation](docs/overview.qmd){.btn .btn-dark .btn-lg .m-2 .px-5 role="button"}
[Component Gallery](components/index.qmd){.btn .btn-dark .btn-lg .m-2 .px-5 role="button"}
:::
